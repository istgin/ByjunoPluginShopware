        ...
        <form action="{url action='confirm'}" method="post" id="proceed_byjuno_installment" name="proceed_byjuno_installment" onsubmit="return byjuno_submit_form_fn()">
            ...
            <script>
                ...
                function byjuno_submit_fn() {
                    setTimeout(function(){ document.getElementById('proceed_byjuno_installment').submit();}, 1);
                }
            </script>
            <button type="button" onclick="byjuno_submit_fn()" class="btn is--primary is--large left is--icon-right"{if count($paymentplans) == 0} disabled="disabled"{/if} form="proceed_byjuno_installment" data-preloader-button="true">{s name=proceed_payment namespace=frontend/byjuno/index}Proceed payment{/s}<i class="icon--arrow-right"></i>
            </button>
        </form>
    </div>
{/block}
